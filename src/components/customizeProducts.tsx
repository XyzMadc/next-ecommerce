          ...
          <ul className="flex items-center gap-4">
            {option.choices?.map((choice) => {
              ...
              return option.name === "Color" ? (
                <li
                  className={`size-6 rounded-full ring-1 ring-gray-300 relative ${
                ...
                >
                  {selected && (
                    <div className="absolute size-8 rounded-full ring-2 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                  )}
                  {disabled && (
                ...
                <li
                  ...
                  className={`ring-1 ring-cart text-cart rounded-md py-1 px-4 text-sm ${
                    selected
                      ...
                      : disabled
                      ? "bg-gray-200 text-white ring-gray-200 cursor-not-allowed"
                      : "bg-white text-red-400 cursor-pointer"
                  }`}
                  onClick={clickHandler}
                >
                  {choice.description}
